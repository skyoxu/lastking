...
func test_preserves_responsibility_boundaries_without_ambiguous_split() -> void:
	...
	assert_bool(summary.text.find("Castle HP:") >= 0).is_true()
...
func test_integration_validates_bottom_bar_state_and_action_readability() -> void:
	...
	assert_bool(summary.text.find("Castle HP:") >= 0).is_true()
	assert_bool(summary.text.find("Friendly Units:") >= 0).is_true()
	assert_bool(summary.text.find("Enemy Units Spawned:") >= 0).is_true()
	assert_bool(summary.text.find("Combat Exchanges:") >= 0).is_true()
	assert_bool(status.text.length() > 0).is_true()
