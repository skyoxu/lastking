...
func test_battle_hud_building_palette_buttons_should_drive_formal_selection_feedback() -> void:
	...
	assert_bool(str(tower_range["overlay_state"]) == "overlay_hidden").is_true()
	assert_bool(str(tower_blocked_range["overlay_state"]) == "overlay_hidden").is_true()
	var range_ring: Control = screen.get_node("Background/BattlefieldViewport/BattlefieldRoot/LocalFeedbackLayer/SelectionRangeRing")
	assert_object(range_ring).is_not_null()
	assert_bool(range_ring.visible).is_true()
	assert_bool(float(range_ring.get("radius_px")) == 300.0).is_true()
