...
Those files are not guaranteed to exist in every copied repository. In `lastking`, the real Taskmaster triplet already exists and should be preferred.
...
- Old `tasks/tasks.json`
  ...
  - `lastking` already has the generated task triplet. Only fall back to template examples when auditing a bare copied repository.
