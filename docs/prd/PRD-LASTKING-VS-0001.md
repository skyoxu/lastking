---
Story-ID: PRD-LASTKING-VS-0001
Title: lastking 首个垂直切片 - 公会管理循环
Status: Active
ADR-Refs:
  - ADR-0018
  - ADR-0004
...
# PRD-LASTKING-VS-0001：首个垂直切片 - 公会管理循环

## 1. 引言与目标（引用 CH01）

### 1.1 Story ID
PRD-LASTKING-VS-0001

### 1.2 目标
定义 lastking 首个可玩的垂直切片：基础公会管理循环（创建公会 → 成员管理 → 解散公会），作为后续功能迭代的基线。
...
- **迁移目标**：反向 DNS 命名（如 `com.lastking.guild.created`）
...
### 7.1 PRD 完整性
- [x] 存在一份针对 lastking 的最小 PRD 片段（本文档）
