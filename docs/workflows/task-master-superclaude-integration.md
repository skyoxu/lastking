...
- **外圈（结构层质量门禁）**：
  - 使用专用 Python 脚本检查契约与文档的一致性和命名规范，例如：
    ...
    - `scripts/python/check_domain_contracts.py`: validates domain EventType naming, uniqueness, and SSoT alignment.
...
- 当某个领域模块（如 Guild、GameLoop）进入“稳定阶段”后：
  - After module rules stabilize, include `check_domain_contracts.py` in local gate routines.
...
- 契约测试：
  - `Game.Core.Tests/Contracts/DomainEventContractTests.cs` verifies event contract shape and EventType usage.
- 自动化校验：
  - `scripts/python/validate_contracts.py` 检查 Overlay 08 中的契约路径是否指向存在的 C# 契约，并已在 `windows-quality-gate.yml` 中以软门禁方式运行。
  - `scripts/python/check_domain_contracts.py` ensures EventType naming stays aligned with ADR-0004.
- 同步更新约定：
  - Keep Overlay contract pages and `Game.Core/Contracts/**` synchronized; validate via `validate_contracts.py` and `check_domain_contracts.py`.
