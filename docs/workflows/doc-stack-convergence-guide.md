...
本手册用于解决仓库文档中残留历史技术栈语境（例如历史运行壳、历史 UI 栈、历史测试/构建工具名）导致的人/LLM 误导问题，并给出在本仓库进行“全量收敛”的可执行步骤与验证方式。
...
- 迁移文档是历史对照资料库，允许保留过程信息，但不得让历史技术栈名词“看起来像当前口径”。如需提及历史实现，使用“历史运行壳/历史 UI 栈/历史项目”等中性描述，避免使用具体产品名。
...
默认扫描关键词（可按需扩展脚本）：
- 旧项目名、历史运行壳、历史 UI 框架、旧构建/测试/E2E 工具等（大小写不敏感）
