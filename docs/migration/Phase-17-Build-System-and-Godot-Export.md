...
> 本节描述的是 **当前 lastking 模板已经落地的构建/导出能力**。下文所述的 `scripts/build_windows.py`、独立 Release 工作流等仍处于蓝图阶段，对应增强统一收敛到 Phase-17 Backlog。
...
# 输出：dist/lastking-1.0.0-debug.exe
...
# 输出：dist/lastking-1.0.0.exe, dist/build-metadata.json, dist/lastking-*-SHA256.txt
