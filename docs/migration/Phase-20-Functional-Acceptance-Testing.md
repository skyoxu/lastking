...
> **核心目标**：逐功能对标验证 LegacyProject 与 lastking 的特性一致性，建立功能完整性与兼容性验收基线。
> **工作量**：5-7 人天
...
| 功能模块 | LegacyProject 实现 | lastking 对标 | 验收标准 |
|---------|-------------|---------------|---------|
...
| UI 组件 | LegacyProject | lastking | 验收标准 |
|--------|---------|----------|---------|
...
| 数据类型 | LegacyProject | lastking | 验收标准 |
|--------|---------|----------|---------|
...
| **数据导入** | - | 迁移脚本 | LegacyProject 数据可导入 lastking |

#### 维度 D: 性能基线对标（Performance Baseline）

| 性能指标 | LegacyProject 基线 | lastking 目标 | 验收标准 |
|--------|-------------|--------------|---------|
...
┌──────────────────────────────────────────────────────┐
│         Phase 20 功能验收测试启动                      │
│  依赖：Phase 1-19 完整实现 + lastking 可运行构建      │
└────────────────────┬─────────────────────────────────┘
                     │
        ┌────────────▼────────────┐
        │  第一阶段：冒烟测试      │
...
**职责**：
- 验证 lastking 核心游戏机制与 vitagame 的一致性
- 对标菜单、场景、数据系统等关键功能
...
namespace Game.Tests.Acceptance
{
    /// <summary>
    /// Phase 20 功能对标测试
    /// 验证 lastking 核心功能与 LegacyProject 的完整性与行为一致性
    /// </summary>
...
**职责**：
- 逐功能列出 LegacyProject vs lastking 的对标要点
- 记录每个功能的验收状态
- 作为发版前的最终检查清单
...
| 指标 | LegacyProject 基线 | lastking 目标 | 实际值 | 状态 |
|------|-------------|--------------|-------|------|
| 启动时间 | <2.0s | <2.5s | ____ | - |
| 场景加载 | <1.0s | <1.2s | ____ | - |
| FPS | 60 | 60 | ____ | - |
