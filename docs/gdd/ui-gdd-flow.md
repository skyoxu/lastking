...
| Combat Pressure And Interaction | player-facing | T04, T05, T06, T20, T22, T43, T47, T48, T49, T50, T51, T52, T53, T54, T67 | Render enemy pressure, targeting, combat outcomes, playable battle-map routing, daily-settlement resolution, and camera interaction without hidden state | Combat HUD / Pressure / Camera Feedback / Battle Map Screen |
...
- T54 `Battle Map Screen Route And Playable Runtime Wiring`
- T67 `Implement Daily Settlement Outcome Modal`
...
| Combat Pressure And Interaction (T04, T05, T06, T20, T22, T43, T47, T48, T49, T50, T51, T52, T53, T54, T67) | Combat HUD / Pressure / Camera Feedback / Battle Map Screen | Fight, observe pressure, targeting, pathing, battle-map flow, daily-settlement resolution, and camera responses | Render enemy pressure, targeting, combat outcomes, playable battle-map routing, daily-settlement resolution, and camera interaction without hidden state | `Game.Core.Tests/Services/WaveManagerBudgetChannelTests.cs`, `Game.Core.Tests/Services/WaveManagerDeterminismTests.cs`, `Game.Core.Tests/Engine/GameEngineCoreDeterminismTests.cs`, `Game.Core.Tests/Services/WaveBudgetAllocatorTests.cs`, `Tests.Godot/tests/Integration/test_singleplayer_startup_flow.gd`, `Tests.Godot/tests/Integration/test_battle_map_screen_runtime_flow.gd`, `Tests.Godot/tests/Integration/test_combat_experience_runtime_flow.gd` |
...
- Combat Pressure And Interaction: define concrete scene ownership, empty/failure states, and validation evidence for T04, T05, T06, T20, T22, T43, T47, T48, T49, T50, T51, T52, T53, T54.
...
- Matrix link: `## 5. UI Wiring Matrix row Combat Pressure And Interaction (T04, T05, T06, T20, T22, T43, T47, T48, T49, T50, T51, T52, T53, T54, T67)`.
- Scope: T04, T05, T06, T20, T22, T43, T47, T48, T49, T50, T51, T52, T53, T54, T67.
- UI entry: Combat HUD / Pressure / Camera Feedback / Battle Map Screen.
- Candidate type: task-shaped UI wiring spec.
- Screen group: Combat Pressure And Interaction Surfaces.
- Player action: Fight, observe pressure, targeting, pathing, battle-map flow, daily-settlement resolution, and camera responses.
- System response: Render enemy pressure, targeting, combat outcomes, playable battle-map routing, daily-settlement resolution, and camera interaction without hidden state.
- Empty state: Wave generation preserves deterministic replay for the same day/config/seed, and wave-budget computation is isolated to wave inputs without cross-system side effects.
- Failure state: Taskmaster #6 is complete only when combat runs demonstrate target selection by the required priority chain and blocked-path fallback attacks whenever routes to priority targets are obstructed.
- Completion result: Changing only one channel budget must change that channel's input budget and alter at least one spawn decision for the same day and seed, while non-target channel outputs remain unchanged.
- Requirement IDs: `RQ-CAMERA-SCROLL`, `RQ-CH7-COMBAT-WIRING`, `RQ-COMBAT-CLEANUP-AFTERACTION`, `RQ-COMBAT-CLEANUP-AFTERACTION`, `RQ-COMBAT-PROJECTILE-AOE`, `RQ-COMBAT-PROJECTILE-AOE`, `RQ-COMBAT-TARGETING-DEPLOYMENT`, `RQ-COMBAT-TARGETING-DEPLOYMENT`, `RQ-COMBAT-TARGETING-DEPLOYMENT`, `RQ-COMBAT-QUEUE-TECH`, `RQ-CORE-LOOP-STATE`, `RQ-CORE-LOOP-STATE`, `RQ-CORE-LOOP-STATE`, `RQ-CH7-BATTLEMAP-WIRING`.
- Validation artifact targets: `logs/ci/<YYYY-MM-DD>/task-triplet-audit/report.json`, `logs/unit/<YYYY-MM-DD>/coverage.json`, `logs/e2e/<YYYY-MM-DD>/runtime-ui/summary.json`, `logs/ci/<YYYY-MM-DD>/chapter7-ui-wiring/closure-summary.json`.
- Suggested standalone surfaces: `CombatHud`, `PressurePanel`, `CameraControlOverlay`, `BattleMapScreen`.
- Test refs: `Game.Core.Tests/Services/WaveManagerBudgetChannelTests.cs`, `Game.Core.Tests/Services/WaveManagerDeterminismTests.cs`, `Game.Core.Tests/Engine/GameEngineCoreDeterminismTests.cs`, `Game.Core.Tests/Services/WaveBudgetAllocatorTests.cs`, `Tests.Godot/tests/Integration/test_singleplayer_startup_flow.gd`, `Tests.Godot/tests/Integration/test_battle_map_screen_runtime_flow.gd`, `Tests.Godot/tests/Integration/test_combat_experience_runtime_flow.gd`.
...
- Requirement IDs: `RQ-CAMERA-SCROLL`, `RQ-CH7-COMBAT-WIRING`, `RQ-COMBAT-CLEANUP-AFTERACTION`, `RQ-COMBAT-PROJECTILE-AOE`, `RQ-COMBAT-TARGETING-DEPLOYMENT`, `RQ-COMBAT-QUEUE-TECH`, `RQ-CORE-LOOP-STATE`.
- Expected artifacts: `logs/ci/<YYYY-MM-DD>/task-triplet-audit/report.json`, `logs/unit/<YYYY-MM-DD>/coverage.json`, `logs/e2e/<YYYY-MM-DD>/runtime-ui/summary.json`, `logs/ci/<YYYY-MM-DD>/chapter7-ui-wiring/closure-summary.json`.
...
- Completed task count currently expected by Chapter 7: 53.
