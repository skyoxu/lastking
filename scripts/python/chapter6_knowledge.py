...
def _validate_semantic_entries(model: object, entries: list[dict]) -> tuple[bool, list[dict], str | None]:
    ...
    for item in model:
        ...
        fields_by_pointer = {str(field.get('pointer')): field for field in fields or [] if isinstance(field, dict) and isinstance(field.get('pointer'), str)}
        ...
        clean = dict(item); clean['id'] = source.get('id'); clean['path'] = path; clean['kind'] = source.get('kind'); clean['parameters'] = validated_parameters
        ...
        if source.get('kind') == 'asset':
            ...
            for binding in bindings:
                if not str(binding.get('meaning') or '').strip(): return False, [], f'Binding meaning is required for {path}'
                key = (binding.get('source'), binding.get('line'))
                if key not in available_bindings: return False, [], f'Unknown asset binding for {path}: {key}'
            ...
            for binding in bindings:
                ...
                if key not in available_bindings: return False, [], f'Unknown scene binding for {path}: {key}'
                actual = available_bindings[key]
...
def _build_semantic_prompt(prompt_entries: list[dict]) -> str:
    return ('Return a JSON array only. Explain each supplied config, asset, or scene resource for a developer implementing this task. Write all explanation fields in English. For each item keep id and path, then add explanation, modification_guidance, modification_impact, parameters, and bindings. parameters must be an array of objects with pointer and meaning. Use only exact pointer values listed in available_parameters for that same config; otherwise use an empty array. bindings must use exact source/line pairs for assets or exact node_path/line pairs for scenes from available_bindings; configs use an empty array. Explain asset replacement constraints and scene/node editing impact without claiming runtime observation. Every returned binding must have a non-empty meaning. Return at least one relevant asset and one relevant scene when those kinds are supplied. Select only task-relevant fields and bindings. Do not invent paths, fields, nodes, runtime observations, or evidence. Omit unrelated resources.\n' + json.dumps(prompt_entries, ensure_ascii=False, separators=(',', ':')))
...
def _semantic_enrich(root: Path, task_id: str, backend: str) -> dict:
    ...
    scenes = [entry for entry in entries if entry.get('kind') == 'scene']; scenes.sort(key=lambda entry: entry.get('evidence', [{}])[0].get('focus') != 'core'); candidates += scenes[:8]
    prompt_entries = _semantic_prompt_entries(candidates); prompt = _build_semantic_prompt(prompt_entries)
    out = root / 'docs/knowledge/generated' / f'task-{task_id}-semantic.json'
    rc, trace, command = 1, '', []; validation_error = None; model = None
    for attempt in range(1, 6):
        rc, trace, command = run_llm_exec(backend=backend, root=root, prompt=prompt, output_last_message=out, timeout_sec=300)
        if rc == 0:
            try:
                candidate = json.loads(out.read_text(encoding='utf-8')); valid, model, validation_error = _validate_semantic_entries(candidate, entries)
                if valid: break
            except (OSError, json.JSONDecodeError) as exc: validation_error = str(exc)
