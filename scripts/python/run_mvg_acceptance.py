...
DEFAULT_MANIFEST = 'docs/testing/mvg/battlemap-pilot.json'
    ...
    try:
        ...
        if args.mode != 'run':
            ...
            if args.challenge_input:
                targets = [test for test in doc['tests'] if test.get('challenge') == 'disable-battlemap-wave-input']
                if not targets:
                    raise ValueError('Manifest has no input challenge target')
                env['MVG_INPUT_CHALLENGE'] = 'disable-battlemap-wave-input'
                summary['challenges'] = []
                for test in targets:
                    ...
                    detected = (result['exit_code'] not in {0, 124, 127}
                                ...
                                and any('MVG_BATTLEMAP_INPUT_DID_NOT_SPAWN_WAVE' in message
                    ...
                    if not detected:
                        raise RuntimeError('Disabled BattleMap wave input was not demonstrated by a test assertion')
