#!/usr/bin/env python3
"""Optional target-native seeded mutation experiment for lastking BattleMap placement boundaries."""
...
SOURCE = 'Game.Core/Services/Building/BuildingPlacementService.cs'
MUTANTS = [
    ('reject-exact-cost-placement', 'state.Resources < building.Cost', 'state.Resources <= building.Cost'),
    ('reverse-placement-charge', 'state.Resources -= building.Cost;', 'state.Resources += building.Cost;'),
]
TEST = {'id': 'battlemap-mutation-probes', 'kind': 'dotnet',
        'evidence_level': 'domain-integration',
        'selector': 'Game.Core.Tests.Integration.MvgBattleMapIntegrationTests', 'min_tests': 3}
