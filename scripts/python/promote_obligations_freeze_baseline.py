...
Flow:
1) Read draft whitelist JSON.
2) Create immutable dated baseline file (fail if exists).
3) Update current baseline pointer file.
4) Emit promotion report to logs/ci/<date>/.
"""
...
def main() -> int:
    ...
    draft_path = repo_root / Path(args.draft)
    baseline_dir = repo_root / Path(args.baseline_dir)
    current_path = repo_root / Path(args.current)
    report_path = repo_root / Path(args.report)
    ...
    if baseline_path.exists():
        print(f"ERROR: immutable baseline already exists: {baseline_path}")
        return 3
