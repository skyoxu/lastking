...
def _build_input_hash_for_task(task_id: str, security_profile: str) -> tuple[str, dict[str, Any]]:
    ...
    first_acceptance_excerpt = ""
    first_view_items = acceptance_by_view.get(first_view) or []
    if first_view_items:
        first_item = first_view_items[0]
        if isinstance(first_item, str):
            first_acceptance_excerpt = str(first_item).strip()
        elif isinstance(first_item, dict):
            first_acceptance_excerpt = str(first_item.get("text") or first_item.get("title") or "").strip()
    if not first_acceptance_excerpt:
        first_acceptance_excerpt = "acceptance"
    return build_input_hash(input_fingerprint), {
        ...
        "first_acceptance_excerpt": first_acceptance_excerpt,
    }
    ...
    obligations: list[dict[str, Any]] = [
        {
            ...
            "source_excerpt": source_excerpt,
            "covered": True,
            "matches": [{"view": first_view, "acceptance_index": 1, "acceptance_excerpt": first_acceptance_excerpt}],
            "reason": "seed",
            "suggested_acceptance": [],
        }
    ]

    for index, item in enumerate(subtasks, start=2):
        sid = str(item.get("id") or "").strip()
        if not sid:
            continue
        excerpt = str(item.get("title") or "").strip() or str(item.get("details") or "").strip() or f"subtask-{sid}"
        obligations.append(
            {
                "id": f"O{index}",
                "source": f"subtask:{sid}",
                "kind": "core",
                "text": f"Seed subtask obligation {sid}.",
...
def main() -> int:
    ...
    _seed_reuse_entry(
        ...
        first_acceptance_excerpt=str(ctx.get("first_acceptance_excerpt") or "acceptance"),
