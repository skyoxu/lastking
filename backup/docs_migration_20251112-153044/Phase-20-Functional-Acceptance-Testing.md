...
> **核心目标**：逐功能对标验证 vitegame 与 lastking 的特性一致性，建立功能完整性与兼容性验收基线。
> **工作量**：5-7 人天
...
| 功能模块 | vitegame 实现 | lastking 对标 | 验收标准 |
|---------|-------------|---------------|---------|
...
| **数据导入** | - | 迁移脚本 | vitegame 数据可导入 lastking |

#### 维度 D: 性能基线对标（Performance Baseline）

| 性能指标 | vitegame 基线 | lastking 目标 | 验收标准 |
|--------|-------------|--------------|---------|
...
namespace Game.Tests.Acceptance
{
    /// <summary>
    /// Phase 20 功能对标测试
    /// 验证 lastking 核心功能与 vitegame 的完整性与行为一致性
    /// </summary>
...
**职责**：
- 逐功能列出 vitegame vs lastking 的对标要点
- 记录每个功能的验收状态
- 作为发版前的最终检查清单
...
| 指标 | vitegame 基线 | lastking 目标 | 实际值 | 状态 |
|------|-------------|--------------|-------|------|
| 启动时间 | <2.0s | <2.5s | ____ | ⚪ |
| 场景加载 | <1.0s | <1.2s | ____ | ⚪ |
| FPS | 60 | 60 | ____ | ⚪ |
