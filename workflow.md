...
MVG 集成补充：单任务期间逐步实现链路测试；MVG 收尾对整合提交运行完整清单，保存运行证据。影响建议只能决定优先顺序，不能删减 required tests。见 [MVG 集成验收](docs/workflows/mvg-integration-acceptance.md)。
...
MVG 集成补充：关键旅程使用真实场景准备、引擎输入、有界状态等待和清理；不得把静态接线或业务方法调用等同 engine-input 验证。见 [MVG 集成验收](docs/workflows/mvg-integration-acceptance.md)。
